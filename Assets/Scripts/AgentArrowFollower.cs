...
public class AgentArrowFollower : MonoBehaviour
{
    ...
    [Header("병목 방지")]
    [Tooltip("목적지를 중심에서 이 반경만큼 랜덤 분산")]
    public float jitterRadius = 1.2f;
        ...
        if (headingToExitDoor)
        {
            if (evac != null && evac.exitPoint != null)
            {
                if (!navAgent.pathPending && (navAgent.remainingDistance == Mathf.Infinity || navAgent.remainingDistance <= navAgent.stoppingDistance + 0.1f))
                    navAgent.SetDestination(JitteredPosition(evac.exitPoint.position));
            }
            return;
        }
    ...
    void BeginFollowing()
    {
        ...
        if (currentTarget != null)
        {
            ...
            navAgent.SetDestination(JitteredPosition(currentTarget.transform.position));
    ...
    {
        StopAllCoroutines();
        CancelInvoke(nameof(BeginFollowing));
        following = false;
        headingToExitDoor = false;
        currentTarget = null;

        if (navAgent != null)
            navAgent.isStopped = true;

        // AgentMover가 없는 경우 Start()는 재호출되지 않으므로 여기서 직접 재시작
        if (startAutomatically && agentMover == null)
            Invoke(nameof(BeginFollowing), Mathf.Max(0f, startDelay));
    }

    Vector3 JitteredPosition(Vector3 target)
    {
        Vector3 jitter = Random.insideUnitSphere * jitterRadius;
        jitter.y = 0f;
        Vector3 jittered = target + jitter;

        NavMeshHit hit;
        if (NavMesh.SamplePosition(jittered, out hit, 2.0f, NavMesh.AllAreas))
            return hit.position;
        return target;
    }
