...
public class FireSpreadManager : MonoBehaviour
{
    ...
    void SpawnFire(Vector3 snappedPos)
    {
        ...
        var fireObj = Instantiate(firePrefab, spawnPos, Quaternion.identity);
        
        // NavMeshObstacle 추가로 불을 장애물로 설정
        var obstacle = fireObj.AddComponent<NavMeshObstacle>();
        obstacle.shape = NavMeshObstacleShape.Box;
        obstacle.size = new Vector3(gridSize, 1f, gridSize); // 그리드 크기에 맞춤
        obstacle.center = Vector3.zero; // 중심에 맞춤
    ...
    /// <summary> 모든 불 위치 목록을 반환합니다. </summary>
    public HashSet<Vector3> GetFirePositions()
    {
        return new HashSet<Vector3>(firePositions);
    }
